...
```
이 레포는 PM이 MD 정책서를 기반으로 프로토타입을 만드는 Next.js 프로젝트야.
아래 스펙대로 전체 환경을 세팅해줘.
...
4. app/api/spec/route.ts — 로컬 MD 파일 읽기 API
5. app/layout.tsx — 루트 레이아웃
6. app/page.tsx — 서비스 인덱스 (gentle-monster, nuflaat, atiissu, common)
7. components/MdDrawer.tsx — ? 버튼 클릭 시 열리는 우측 드로어 (specs/ MD 렌더링)
8. components/PolicyButton.tsx — ? 버튼 컴포넌트
9. mock-data/index.ts — 목업 데이터 (Order, Product 타입 포함)
10. specs/ — 정책서 MD 파일 폴더 (app/ 경로와 동일 구조)
11. CLAUDE.md — PM용 작업 지침
12. README.md — PM 온보딩 가이드
...
## MD 드로어 동작
- PolicyButton의 specFile prop에 specs/ 기준 경로 지정 (예: "common/oms/dashboard")
- 클릭 시 /api/spec?file=xxx 호출 → specs/{file}.md 파일 읽기
- 우측에서 슬라이드로 열리는 드로어에 MD 내용 렌더링
```
...
- [ ] Vercel 연동 (환경변수 `ACCESS_PASSWORD` 등록)
- [ ] Slack 알림 연동
