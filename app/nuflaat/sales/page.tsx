...
const pages: { name: string; path: string; specPath: string; description: string; status: 'ready' | 'planned' }[] = [
  {
    ...
    path: '/nuflaat/sales/dashboard',
    specPath: '/specs/nuflaat/sales/dashboard',
    description: '온라인 통합 판매 데이터 원페이지 대시보드',
    status: 'ready',
  },
]
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">온라인 통합 판매 데이터 분석 · 데이터 통합·표준화·자동화 기반</p>
        </div>
        ...
        <section>
          ...
          {pages.length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-gray-300 px-6 py-10 text-center text-gray-400 text-sm">
              아직 등록된 화면이 없습니다. specs/nuflaat/sales/ 에 정책서를 추가하세요.
            </div>
          ) : (
